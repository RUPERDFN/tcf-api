...
import { shoppingItems, menus, gamification, pointsLog, profiles } from '../../drizzle/schema.js';
import { shoppingItemSchema } from '../middleware/validation.middleware.js';
import { AuthRequest } from '../types/index.js';
import skinChefService from '../services/skinchef.service.js';
...
export async function getSubstitution(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    ...
    const substitutions = await skinChefService.getSubstitutions(
      userId,
      {
        budget: profile?.budgetWeekly || 50,
        diners: profile?.diners || 2,
        mealsPerDay: profile?.mealsPerDay || 3,
        dietType: profile?.dietType || 'omnivora',
