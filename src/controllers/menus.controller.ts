...
import { menus, profiles, completedMeals, gamification, pointsLog, shoppingItems, userMetrics } from '../../drizzle/schema.js';
import { menuGenerateSchema, completeMealSchema } from '../middleware/validation.middleware.js';
import { AuthRequest } from '../types/index.js';
import skinChefService, { MealItem, ShoppingItem } from '../services/skinchef.service.js';
import { computeMenuMacros } from '../services/macros.service.js';
...
export async function generate(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    ...
    // Fetch AI preference vector for personalized generation
    const [metrics] = await db.select().from(userMetrics).where(eq(userMetrics.userId, userId));
    const preferenceVector = metrics?.preferenceScore as Record<string, number> | undefined;

    const menuData = await skinChefService.generateMenu(
      userId,
      {
        ...
        preferenceVector // AI personalization injection
      },
      days
    );
...
export async function swap(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    ...
    // Fetch AI preference vector for personalized swap
    const [swapMetrics] = await db.select().from(userMetrics).where(eq(userMetrics.userId, userId));
    const preferenceVector = swapMetrics?.preferenceScore as Record<string, number> | undefined;

    const result = await skinChefService.swapMeal(
      userId,
      {
        budget: profile?.budgetWeekly || 50,
        diners: profile?.diners || 2,
        mealsPerDay: profile?.mealsPerDay || 3,
        dietType: constraints?.dietType || profile?.dietType || 'omnivora',
        allergies: profile?.allergies || [],
        dislikes: profile?.dislikes || [],
        pantryItems: profile?.pantryItems || [],
        preferenceVector // AI personalization injection
    ...
    const swappedMenuData = result.menu || menuData;
    if (swappedMenuData[dayIndex]?.meals?.[mealType]) {
      menuData[dayIndex].meals[mealType] = swappedMenuData[dayIndex].meals[mealType];
    }

    await db.delete(shoppingItems).where(eq(shoppingItems.menuId, menuId));
...
export async function getRecipeEndpoint(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    ...
    const recipe = await skinChefService.getRecipe(meal.name);
...
export async function deleteMenu(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    ...
    await db.delete(completedMeals).where(eq(completedMeals.menuId, menuId));
    await db.delete(shoppingItems).where(eq(shoppingItems.menuId, menuId));
...
// Auto-Macros: compute per-meal and daily macro totals for a stored menu
export async function getMenuMacros(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    const userId = req.user!.userId;
    const menuId = parseInt(req.params.menuId);
    ...
    if (!Array.isArray(menuData) || menuData.length === 0) {
      return res.status(400).json({ error: 'El menú no contiene datos de comidas' });
    }

    const report = await computeMenuMacros(menuData);
