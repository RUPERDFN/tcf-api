...
import { users, profiles, gamification, pointsLog, passwordResetTokens, tenants, subscriptions, userMetrics } from '../../drizzle/schema.js';
...
export async function register(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const [tenant] = await db.insert(tenants).values({
      name: `Familia de ${data.name || data.email.split('@')[0]}`,
      plan: 'FREE'
    }).returning();
    ...
    await db.insert(userMetrics).values({ userId: user.id });
...
export async function me(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    ...
    const [tenant] = user.tenantId ? await db.select().from(tenants).where(eq(tenants.id, user.tenantId)) : [null];
