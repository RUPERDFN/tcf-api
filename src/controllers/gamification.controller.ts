...
import { gamification, pointsLog, users } from '../../drizzle/schema.js';
import type { BadgeEntry } from '../../drizzle/schema.js';
...
export async function getStats(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    ...
    const savings = await GamificationService.getSavings(userId);
    const culinaryProfileName = await GamificationService.getCulinaryLevel(userId);
...
export async function getSundayReview(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    const userId = req.user!.userId;
    const review = await GamificationService.getSundayReview(userId);
    res.json(review);
  } catch (error) {
    next(error);
  }
